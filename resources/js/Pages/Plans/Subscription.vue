...
<template>
  <AuthenticatedLayout>
    ...
    <Head :title="`| ${$t('plans.Purchase')} - ${$t(`plans.${plan.name}`).toUpperCase()}`" />
    <template #header>
      <div v-if="message" class="max-w-2xl mx-auto text-center">
        <!-- <h2 class="text-xl font-bold lg:text-2xl text-primary">Pay now your purchase.</h2> -->
        <n-alert title="Card Error!" type="error" closable>
          {{ message }}
        </n-alert>
      </div>
    </template>
    ...
    <div class="pb-12">
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        <div class="flex flex-wrap items-stretch justify-center sm:-mx-4">
          <div class="w-full mb-8 px-2 sm:px-4 sm:w-4/5 md:w-3/5 xl:w-1/3 lg:mb-0">
            <div class="h-full flex flex-col p-6 space-y-6 rounded shadow sm:p-8 bg-white border border-solid border-gray-300">
              <div class="space-y-3">
                ...
                <span v-if="$page.props.auth.user.promo_code && plan.account_type == 0" class="text-4xl font-bold">${{
                  plan.price * 0.8 }}</span>
                <span class="text-xl text-gray-600" v-if="plan.account_type == 1 && plan.slug != 'Beginner'">/Employee x
                  {{ form.count }} = ${{
              ...
              <div class="mt-4" v-if="plan.account_type == 1 && plan.slug != 'Beginner'">
                ...
                <TextInput id="count" type="number" min="10" class="mt-1 block w-full" v-model="form.count" required />
